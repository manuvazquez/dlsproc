...
# Cell
def merge(history_file: str | pathlib.Path, zip_file: str | pathlib.Path, output_file: str | pathlib.Path | None = None) -> None | pd.DataFrame:
    ...
    if output_file:
        ...
        parquet_df = dlsproc.assemble.parquet_amenable(reindexed_concatenated_df)

        parquet_df.to_parquet(output_file)

    else:

        return reindexed_concatenated_df